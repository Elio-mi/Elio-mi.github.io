...
> 上面分析了这个半天了那么多态的原理到底是什么？还记得这里`Func`函数传`Person`调用的 `Person::BuyTicket`，传Student调用的是`Student::BuyTicket`.<!-- 图片已移除：image-20230731090808387 -->
> ```cpp
> class Person 
> {
> 	public:
...
> ```
>
> 观察下图中的监视窗口中我们发现看不见func3和func4。这里是编译器的监视窗口故意隐藏了这 两个函数，也可以认为是他的一个小bug。那么我们如何查看d的虚表呢？下面我们使用代码打印 出虚表中的函数。<!-- 图片已移除：image-20230731093846804 -->	
> ```cpp
> typedef void(*VFPTR) ();
